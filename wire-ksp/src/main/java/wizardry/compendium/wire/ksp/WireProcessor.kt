...
class WireProcessor(
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        ...
        // Emit the migrator registry referencing every consecutive version
        // pair (vN → v(N+1)) that has committed schema lock files. The
        // referenced migrator classes must exist on the classpath — either
        // auto-generated by this processor (mechanical changes) or
        // hand-committed under the same FQN (`generated.MigratorVxToVy`)
        // after the developer fills in non-mechanical bodies.
        emitMigratorRegistry(currentVersion = snapshot.version)
    ...
    /**
     * Emits `wizardry.compendium.wire.generated.WireMigrators` with a list
     * of every `MigratorV{N}ToV{N+1}` class corresponding to consecutive
     * snapshots in the schema dir.
     *
     * # Convention enforced by this generator
     *
     * - All migrators live in `wizardry.compendium.wire.generated`. Auto-
     *   generated ones are emitted there directly. Hand-edited ones (for
     *   non-mechanical changes) must be moved to a committed source file
     *   under the same FQN before they become referenceable across builds.
     * - The class name is `MigratorV{N}ToV{N+1}`. The processor relies on
     *   this naming so it can reference migrators by name without resolving
     *   them — keeping the registry generator independent of whether the
     *   migrator is currently on the classpath.
     *
     * # Why we don't try to discover migrators by symbol resolution
     *
     * Symbol-resolution-based registry building is appealing — "find every
     * `WireMigrator` and emit them" — but it has a chicken-and-egg problem:
     * the migrator we *just* generated isn't yet visible to KSP in the
     * current round. The naming-convention approach sidesteps this: as long
     * as everyone agrees on the package and class-name pattern, the
     * registry is a function of the version count, not the in-flight
     * symbol table.
     */
    private fun emitMigratorRegistry(currentVersion: Int) {
        val schemaDirPath = env.options[OPTION_WIRE_SCHEMA_DIR] ?: return
        val schemaDir = File(schemaDirPath)
        // Versions present on disk plus the current annotated version. We
        // include the latter so an in-progress version bump is reflected
        // even before the dev runs `updateWireSchemaLock`.
        val committedVersions = schemaDir
            .listFiles { f -> f.name.matches(SCHEMA_FILE_REGEX) }
            ?.mapNotNull { f ->
                f.nameWithoutExtension.removePrefix("v").toIntOrNull()
            }
            .orEmpty()
        val allVersions = (committedVersions + currentVersion).toSortedSet()
        if (allVersions.size < 2) {
            // Single version = no migrators yet. Emit an empty registry so
            // runtime code doesn't have to special-case "registry not
            // generated yet."
            writeRegistryFile(emptyList())
            return
        }
        // Pair up consecutive versions: (v1,v2), (v2,v3), ...
        val pairs = allVersions.zipWithNext()
        writeRegistryFile(pairs)
    }
    ...
    companion object {
        ...
        private val SCHEMA_FILE_REGEX = "v\\d+\\.json".toRegex()
