...
object EnvelopeCodec {
    ...
    fun decode(text: String): Envelope {
        ...
        // are pure identity transforms — they don't bump the `v` field. By
        // contract, completing the chain has brought the envelope up to
        // `CurrentVersion`, so set `v` explicitly here as a safety net.
        if (incomingVersion != CurrentVersion) {
